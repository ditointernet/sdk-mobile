...
android_version="$(sed -nE 's/^version[[:space:]]*=[[:space:]]*System\\.getenv\\(\"VERSION_NAME\"\\)[[:space:]]*\\?:[[:space:]]*\"([^\"]+)\".*/\1/p' android/dito-sdk/build.gradle.kts | head -n 1)"
...
ensure_tag() {
  ...
  if git tag --list "${prefix}v*" | grep -q .; then
    return 0
  fi
