...
class DitoMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        if (remoteMessage.data.isNotEmpty()) {
            ...
            try {
                objData?.getString("log_id")?.let { logId = it }
            } catch (e: Exception) {
                Log.d("empty-log-id", "There is no log_id in notification")
            }

            try {
                objData?.getJSONObject("details")?.getString("notification_name")?.let { notificationName = it }
            } catch (e: Exception) {
                Log.d("empty-notification-name", "There is no notification_name in notification")
            }

            try {
                objData?.getString("user_id")?.let { userId = it }
            } catch (e: Exception) {
                Log.d("empty-user-id", "There is no user_id in notification")
            }
            ...
            if (notificationId.isNotEmpty() && reference.isNotEmpty()) {
                ...
                val userInfo = mapOf(
                    ...
                    "log_id" to logId,
                    "notification_name" to notificationName,
                    "user_id" to userId
                )
