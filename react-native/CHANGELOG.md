...
- Método público `DitoSdk.logout(): Promise<void>` para limpar localmente dados persistidos por `identify` e a identidade local usada por tracking, sem remover configuração da SDK, opções de notificação, inbox, tokens de push ou dados remotos no backend.
...
- Metadados React Native alinhados aos SDKs nativos alvo: Android 3.4.0 e iOS 3.3.1.
