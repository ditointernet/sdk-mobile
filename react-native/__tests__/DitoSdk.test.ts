...
const { default: DitoSdk, DitoErrorCode } = require("../src/index");
...
describe("DitoSdk", () => {
  ...
  describe("getPlatformVersion", () => {
    it("should return platform version", async () => {
      mockNativeModule.getPlatformVersion.mockResolvedValue("Android 14");

      const version = await DitoSdk.getPlatformVersion();

      expect(mockNativeModule.getPlatformVersion).toHaveBeenCalledWith();
      expect(version).toBe("Android 14");
    });
  });

  describe("setDebugMode", () => {
    it("should enable debug mode", async () => {
      mockNativeModule.setDebugMode.mockResolvedValue(undefined);

      await DitoSdk.setDebugMode(true);

      expect(mockNativeModule.setDebugMode).toHaveBeenCalledWith(true);
    });

    it("should disable debug mode", async () => {
      mockNativeModule.setDebugMode.mockResolvedValue(undefined);

      await DitoSdk.setDebugMode(false);

      expect(mockNativeModule.setDebugMode).toHaveBeenCalledWith(false);
    });
  });
